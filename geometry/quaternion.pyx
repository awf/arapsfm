...
cdef extern from "Geometry/quaternion.h":
    ...
    void quatInv_Unsafe(double * q, double * x)
    void quatInvDq_Unsafe(double * q, double * Dq)
...
def quatInv(np.ndarray[DTYPE_t, ndim=1] q):
    if q.shape[0] != 4:
        raise ValueError('q.shape[0] != 4')

    cdef np.ndarray[DTYPE_t, ndim=1] x = np.empty(3, dtype=DTYPE)
    quatInv_Unsafe(<double *>q.data, <double *>x.data)

    return x

def quatInvDq(np.ndarray[DTYPE_t, ndim=1] q):
    if q.shape[0] != 4:
        raise ValueError('q.shape[0] != 4')

    cdef np.ndarray[DTYPE_t, ndim=2] Dq = np.empty((3, 4), dtype=DTYPE)
    quatInvDq_Unsafe(<double *>q.data, <double *>Dq.data)
