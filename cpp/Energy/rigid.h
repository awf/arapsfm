...
// BackwardRigidRegistrationEnergy
class BackwardRigidRegistrationEnergy : public Energy
{
public:
    ...
    virtual void EvaluateResidual(const int k, Vector<double> & e) const
    {
        ...
        double Vd[3];
        subtractVectors_Static<double, 3>(_V.GetVertex(k), _d.GetVertex(0), Vd);
        scaleVectorIP_Static<double, 3>(_s.GetScale(), Vd);

        subtractVectors_Static<double, 3>(Vd, RV0, &e[0]);
        scaleVectorIP_Static<double, 3>(_w, &e[0]);
    }
    ...
    virtual void EvaluateJacobian(const int k, const int whichParam, Matrix<double> & J) const
    {
        ...
        case 0:
        {
            // s
            double Vd[3];
            subtractVectors_Static<double, 3>(_V.GetVertex(k), _d.GetVertex(0), J[0]);
            scaleVectorIP_Static<double, 3>(_w, J[0]);
            return;
        }
        case 1:
        {
            ...
            rigidJac_X_Unsafe(_V0.GetVertex(k), _w, q, D, J[0]);
            return;
        }
        case 2:
        {
            // d
            fillVector_Static<double, 9>(0., J[0]);
            J[0][0] = - _s.GetScale() * _w;
            J[1][1] = - _s.GetScale() * _w;
            J[2][2] = - _s.GetScale() * _w;
            return;
        }
        case 3:
        {
