...
void silhouetteNormalResiduals3_Unsafe(const Mesh & mesh, const Matrix<double> & V1, 
                                       int faceIndex, const double * u_, const double * SN, 
                                       const double & w, double * e)
{
    const int * Ti = mesh.GetTriangle(faceIndex);
...
void silhouetteNormalResiduals3Jac_V1_Unsafe(const Mesh & mesh, const Matrix<double> & V1, 
                                             int faceIndex, const double * u_, 
                                             const double * SN,
                                             int vertexIndex, const double & w, double * J)
{
...
void silhouetteNormalResiduals3Jac_u_Unsafe(const Mesh & mesh, const Matrix<double> & V1, 
                                            int faceIndex, const double * u_, const double * SN, 
...
// SilhouetteNormalEnergy3
SilhouetteNormalEnergy3::SilhouetteNormalEnergy3(const VertexNode & V, const BarycentricNode & U, 
                                                 const Matrix<double> & SN, const Mesh & mesh, 
                                                 const double w, const int narrowBand,
                                                 const ResidualTransform * pResidualTransform)
    : SilhouetteBaseEnergy(V, U, mesh, w, narrowBand, 3, pResidualTransform), _SN(SN)
...
// SilhouetteNormalEnergy3
void SilhouetteNormalEnergy3::EvaluateResidual(const int k, Vector<double> & e) const
{
    silhouetteNormalResiduals3_Unsafe(_mesh, 
                                      _V.GetVertices(),
                                      _U.GetFaceIndex(k),
                                      _U.GetBarycentricCoordinate(k),
                                      _SN[k],
...
void SilhouetteNormalEnergy3::EvaluateJacobian(const int k, const int whichParam, Matrix<double> & J) const
{
    if (whichParam == 0)
    {
        // barycentric coordinate
        silhouetteNormalResiduals3Jac_u_Unsafe(_mesh, 
            _V.GetVertices(), 
            _U.GetFaceIndex(k),
            _U.GetBarycentricCoordinate(k),
            _SN[k],
    ...
    else
    {
        ...
        silhouetteNormalResiduals3Jac_V1_Unsafe(_mesh,
            _V.GetVertices(),
            _U.GetFaceIndex(k),
            _U.GetBarycentricCoordinate(k),
            _SN[k],
