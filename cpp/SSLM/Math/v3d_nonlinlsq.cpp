...
//======================================================================
   ...
   NLSQ_LM_Optimizer::setupSparseJtJ()
   {
      ...
      for (int obj = 0; obj < nObjs; ++obj)
      {
         ...
         int const nMeasurements = costFun.numMeasurements();
   ...
   void
   NLSQ_LM_Optimizer::fillJacobians(const int iteration)
   {
      int const nObjs = _costFunctions.size();
      for (int obj = 0; obj < nObjs; ++obj)
      {
   ...
   NLSQ_LM_Optimizer::evalJt_e(Vector<double>& Jt_e)
   {
      ...
      for (int obj = 0; obj < nObjs; ++obj)
      {
         ...
         for (int i = 0; i < nParamTypes; ++i)
         {
            ...
            for (int k = 0; k < nMeasurements; ++k)
            {
               int const id = costFun.correspondingParam(k, i);
   ...
   NLSQ_LM_Optimizer::minimize()
   {
      ...
      int const nObjs = _costFunctions.size();
      int jacobianEvaluationIteration = -1;
      ...
      for (currentIteration = 0; currentIteration < maxIterations; ++currentIteration)
      {
         ...
         if (computeDerivatives)
         {
            ...
            if (optimizerVerbosenessLevel >= 2) 
			{
				cout << "NLSQ_LM_Optimizer: residuals (itemized) = "; 
				displayVector(errors);
			}
         ...
         if (success)
         {
            ...
            double const denom2 = abs(innerProduct(delta, Jt_e));
         ...
         if (success && deltaError < 0)
         {
            if (this->applyImprovementStoppingCriteria(deltaError, err))
            {
               ...
               status = LEVENBERG_OPTIMIZER_SMALL_IMPROVEMENT;
               goto end;
            }
         }
