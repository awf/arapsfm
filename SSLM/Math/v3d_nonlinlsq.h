...
#define NLSQ_MAX_PARAM_TYPES 32
   ...
   struct NLSQ_CostFunction
   {
       NLSQ_CostFunction(std::vector<int> const& usedParamTypes,
                         ...
                         const Matrix<int> * correspondingParams = nullptr)
            : _usedParamTypes(usedParamTypes),
              _measurementDimension(measurementDimension),
              _correspondingParams(correspondingParams)
         {
            if (correspondingParams != nullptr) assert(usedParamTypes.size() == correspondingParams->num_cols());
         }
         ...
         {
            assert (_correspondingParams != nullptr);
            return (*_correspondingParams)[k][i]; 
         }

         virtual int numMeasurements() const
         {
            assert (_correspondingParams != nullptr);
            return _correspondingParams->num_rows(); 
         }

         virtual int numParameters() const
         {
            assert (_correspondingParams != nullptr);
            return _correspondingParams->num_cols(); 
         }
      ...
      protected:
         ...
         {
            for (int k = 0; k < this->numMeasurements(); ++k) this->evalResidual(k, residuals[k]);
         ...
         const Matrix<int> * _correspondingParams;
   ...
   struct NLSQ_LM_Optimizer : public LevenbergOptimizerCommon
   {
         ...
         virtual void beginIteration(const int currentIteration) {}
