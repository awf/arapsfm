                               ...
                               const OptimiserOptions * options)
{
    ...
    PYARRAY_AS_MATRIX(double, npy_X, X);
    auto node_X = new RotationNode(X);
    node_X->SetPreconditioner(preconditioners[1]);
    problem.AddNode(node_X);
    ...
    if (Xp.num_rows() > 0)
    {
        ...
        problem.AddEnergy(new RotationRegulariseEnergy(*node_Xgp, sqrt(Xp.num_rows() * lambdas[3])));
        problem.AddEnergy(new ScaleRegulariseEnergy(*node_sp, sqrt(Xp.num_rows() * lambdas[4])));
        problem.AddEnergy(new RotationRegulariseEnergy(*node_Xp, sqrt(lambdas[5])));
    }
