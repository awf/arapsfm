...
int solve_single_arap_abs(PyArrayObject * npy_V,
                          PyArrayObject * npy_T,
                          PyArrayObject * npy_X,
                          PyArrayObject * npy_V1,
                          PyArrayObject * npy_C,
                          ...
                          const OptimiserOptions * options)
{
    ...
    ARAPEnergy * arapEnergy = new ARAPEnergy(*nodeV, *nodeX, *nodeV1, mesh, sqrt(lambdas[0]));
    AbsolutePositionEnergy * absEnergy = new AbsolutePositionEnergy(*nodeV1, C, P, sqrt(lambdas[1]));

    problem.AddEnergy(arapEnergy);
    problem.AddEnergy(absEnergy);
...
int solve_multiview_nonlinear_basis(
    PyArrayObject * npy_T,
    PyArrayObject * npy_V,
    PyObject * list_multiXg,
    PyObject * list_multis,
    PyObject * list_multiX,
    PyObject * list_multiy,
    PyObject * list_multiV,
    PyObject * list_multiU,
    PyObject * list_multiL,
    PyObject * list_multiS,
    ...
    const OptimiserOptions * options)
{
    ...
    auto multis = PyList_to_vector_of_Matrix<double>(list_multis);
    auto multiX = PyList_to_vector_of_Matrix<double>(list_multiX);
    auto multiy = PyList_to_vector_of_Matrix<double>(list_multiy);
    auto multiV = PyList_to_vector_of_Matrix<double>(list_multiV);
    auto multiU = PyList_to_vector_of_Matrix<double>(list_multiU);
    auto multiL = PyList_to_vector_of_Vector<int>(list_multiL);
    auto multiS = PyList_to_vector_of_Matrix<double>(list_multiS);
    ...
    // basis rotations
    vector<RotationNode *> basisRotationNodes;

    for (auto i = multiX.begin(); i != multiX.end(); ++i)
    {
        basisRotationNodes.push_back(new RotationNode(*(*i)));
        problem.AddNode(basisRotationNodes.back());
    }
    basisRotationNodes.back()->SetPreconditioner(preconditioners[1]);

    int n = multiX.size();

    // instance basis coefficients
    vector<CoefficientsNode *> basisCoefficientNodes;

    for (auto i = multiy.begin(); i != multiy.end(); ++i)
    {
    ...
    for (int i = 0; i < instVertexNodes.size(); ++i)
    {
        problem.AddEnergy(new DualNonLinearBasisArapEnergy(
                          *coreVertexNode, 
                          *instGlobalRotationNodes[i],
                          *instScaleNodes[i], 
                          basisRotationNodes,
                          *basisCoefficientNodes[i],
                          *instVertexNodes[i], 
                          mesh, 
                          sqrt(lambdas[0]), 
                          uniformWeights));
