# solve_multiple_nonlinear_basis.py
...
def main():
    ...
    # basis rotations (X)
    X = [np.zeros_like(V1[0]) for i in xrange(args.num_basis_rotations)]

    # instance coefficients (y)
    y = [np.ones(args.num_basis_rotations)[:, np.newaxis] for v in V1]
    ...
    def solve_iteration(i):
        ...
        for l, index in enumerate(indices):
            ...
            d = dict(T=T, V=V1[l], Xg=Xg[l], s=instScales[l], X=X, y=y[l], 
